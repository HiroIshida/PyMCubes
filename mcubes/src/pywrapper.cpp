    ...
    int numx, int numy, int numz, PyObject* pyfunc, double isovalue)
{
    ...
    npy_intp size_neighbors = neighbors.size();
    ...
    PyArrayObject* neighborsarr = reinterpret_cast<PyArrayObject*>(PyArray_SimpleNew(1, &size_neighbors, PyArray_ULONG));
    ...
    std::vector<size_t>::const_iterator it3 = neighbors.begin();
    for(int i=0; it3!=neighbors.end(); ++i, ++it3)
        *reinterpret_cast<unsigned long*>(PyArray_GETPTR1(neighborsarr, i)) = *it3;
    
    PyObject* res = Py_BuildValue("(O,O,O)", verticesarr, polygonsarr, neighborsarr);
    Py_XDECREF(verticesarr);
    Py_XDECREF(polygonsarr);
    Py_XDECREF(neighborsarr);
    return res;
}
